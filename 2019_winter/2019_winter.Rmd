...
g <- g + geom_boxplot(mapping = aes_string(x=x_axis, y=y_axis, group='m_jun'), alpha=0.4, notch = TRUE, na.rm = TRUE, colour = '#00009999',data=result_m_d %>% filter(y_num > 2008) %>% group_by(m_jun))
g <- g + geom_point(mapping = this_aes, alpha=1, na.rm = TRUE, colour = '#99000099',data=result_m_d %>% filter((NA_y=='2019' & NA_m==12) | (NA_y=='2020' & as.numeric(as.character(NA_m))<3)))
g <- g + theme(axis.text.x = element_text(angle = 90, hjust = 1))
plot(g)
```

```{r}
pdf_filename <- paste('sakata_winter_2019_', y_axis, '.pdf') 
quartz(type='pdf', file=pdf_filename)
plot(g)
dev.off()
```
