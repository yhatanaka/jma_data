...
testData_predict %<>% mutate(
  predict_stm = case_when(
    str_c(pred_last_year, st_date %>% format(format = "/%m/%d")) %>% as.Date > pred_last_day$st_date %>% as.Date ~ DTS,
    str_c(pred_last_year, st_date %>% format(format = "/%m/%d")) %>% as.Date == pred_last_day$st_date %>% as.Date ~ pred_last_day$DTSsum,
    TRUE ~ 0
  )
)
