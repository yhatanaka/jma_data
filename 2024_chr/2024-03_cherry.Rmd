...
this_aes <- aes(x=.data[[x_axis]], y=.data[[y_axis]], colour=effect_y, group=effect_y)
this_aes2 <- aes(x=.data[[x_axis]], y=predict_stmsum, group=effect_y)
...
# 今年の予想範囲
# , linetype="dashed", 
g <- g + geom_path(mapping = this_aes2, na.rm = TRUE, alpha=0.1,linewidth=1, data = testData_predict %>% filter(effect_y != thisYear))
# 前年までのデータ
...
# 2007年以降の開花日の箱ヒゲ図
g <- g + geom_boxplot(mapping = aes(x=.data[[x_axis]], y=.data[[y_axis]]),lwd=0.4, alpha=0.4, notch = TRUE, na.rm = TRUE, colour = '#00009999',data=testData_actual %>% filter(effect_y %>% as.character() %>% as.integer() > 2007))
# 開花日の散布図
...
g <- g + geom_boxplot(mapping = aes(x=.data[[x_axis]], y=predict_stmsum, group=.data[[x_axis]]),lwd=0.1, alpha=0.4, notch = TRUE, na.rm = TRUE, colour = '#00009999', outlier.size = 0.5, data=testData_predict)

g <- g + geom_path(mapping = aes(x=st_date, y=DTSsum), na.rm = TRUE, alpha=1,data = predict_line, linetype="dotted", colour="red", linewidth=0.2)
