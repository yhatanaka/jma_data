...
result_m_d %<>% mutate(y_num = as.numeric(as.character(NA_y)))
result_m_d %<>% mutate(
  jun = case_when(
...
```{r}
result_m_d_min_max <- result_m_d %>% filter(NA_y == 2019) %>% select(NA_m, NA_d, 平均気温, 最高気温, 最低気温)
pickup_jun_temp <- function(tbl,str){
  return(tbl %>% select(str) %>% as.matrix() %>% mean())
}
result_m_d_min_max %>% filter(NA_m == '8') %>% filter(as.numeric(as.character(NA_d)) < 10)
temp_defer <- function(tbl,str){
  temp_min <- tbl %>% filter(NA_m == '7') %>% filter(as.numeric(as.character(NA_d)) > 7) %>% filter(as.numeric(as.character(NA_d)) < 15) %>% pickup_jun_temp(str)
  temp_max <- tbl %>% filter(NA_m == '8') %>% filter(as.numeric(as.character(NA_d)) < 10) %>% pickup_jun_temp(str)
  return(c(temp_max,temp_min, temp_max - temp_min))
}
result_m_d_min_max %>% temp_defer('最高気温')
result_m_d_min_max %>% temp_defer('平均気温')
result_m_d_min_max %>% temp_defer('最低気温')
```
...
g <- g + geom_point(mapping = this_aes, alpha=1, na.rm = TRUE, colour = '#99000099',data=result_m_d %>% filter(NA_y=='2019'))
g <- g + geom_point(mapping = this_aes, alpha=0.4, na.rm = TRUE, colour = '#99009999',data=result_m_d %>% filter(NA_y=='2018'))
g <- g + geom_boxplot(mapping = aes_string(x=x_axis, y=y_axis, group='m_jun'), alpha=0.4, notch = TRUE, na.rm = TRUE, colour = '#00009999',data=result_m_d %>% filter(y_num > 2008) %>% filter(y_num != 2019) %>% group_by(m_jun))
g <- g + theme(axis.text.x = element_text(angle = 90, hjust = 1))
plot(g)
```

```{r}
quartz(type='pdf', file='sakata_summer_high.pdf')
plot(g)
dev.off()
```
