...
thisPlaceData <- allData %>% mutate(m_d = str_c(start_m %>% padding_date, start_d %>% padding_date,sep='/') %>% as.character()) %>% group_by(start_y) %>% mutate(降雪積算=cumsum(降雪量合計)) %>% mutate(display_ymd = if_else(start_m %>% as.character == '12', paste('2024',start_m %>% as.character,start_d %>% as.character,sep='-') %>% as.Date, paste('2025',start_m %>% as.character,start_d %>% as.character,sep='-') %>% as.Date)) %>% mutate(ef_y = if_else(start_m %>% as.character == '12', start_y, (start_y %>% as.character %>% as.integer - 1) %>% as.factor))
```
...
thisPlaceData2
this_aes <- aes(x=.data[[x_axis]], y=.data[[y_axis]], colour=ef_y, group=ef_y)
...
```{r}
# 箱ヒゲ図
g <- g + geom_boxplot(mapping = aes(x=.data[[x_axis]], y=.data[[y_axis]], group = display_ymd), lwd=0.2, notch = TRUE, na.rm = TRUE, colour = '#00009966', data = thisPlaceData2 %>% group_by(display_ymd) %>% filter(ef_y != 2024))


# 前年までのデータ
g <- g + geom_path(mapping = this_aes, na.rm = TRUE, alpha = 0.01, data = thisPlaceData2 %>% filter(ef_y != 2024), linetype="dashed")

# 今年のデータ
g <- g + geom_path(mapping = this_aes, na.rm = TRUE, alpha = 1, data = thisPlaceData2 %>% filter(ef_y == 2024))
