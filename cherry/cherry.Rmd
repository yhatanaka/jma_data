...
change_colm_type <- function(tbl){
  ...
  tbl %<>% mutate_at(vars(contains('_')), funs(as.factor)) %>%
    mutate_at(vars(without_underbar_cols), funs(as.numeric)) %>%
    mutate(place = as.factor(place)) %>%
    mutate(st_date = as.Date(st_date))
  return(tbl)
}
...
# この年より後のデータ
thls_year = 1970
result <- moddata_list %>% lapply(format_jma)
# あとで年の大小を基準にしてfilteringするので，y_numは数字に
result %<>% bind_rows() %>%
  mutate(y_num = as.numeric(as.character(NA_y))) %>%
  filter(y_num > thls_year) %>%
  change_colm_type() %>%
  mutate(y_num = as.numeric(as.character(y_num)))
...
result_m_d <- result %>% mutate(m_d = str_c(NA_m %>% padding_date,NA_d %>% padding_date,sep='/') %>% as.factor)
# result_m_d %<>% mutate(y_type = if_else(condition = NA_y == 2019, "this", "prev"))
result_m_d %<>% mutate(
  jun = case_when(
    as.numeric(as.character(NA_d)) < 11 ~ '上旬',
    between(as.numeric(as.character(NA_d)), 11, 20) ~ '中旬',
    between(as.numeric(as.character(NA_d)), 21, 31) ~ '下旬'
  )
)
result_m_d %<>% mutate(m_jun = str_c(as.character(NA_m),jun,sep='月'))
...
g <- g + geom_point(mapping = this_aes, alpha=0.4, na.rm = TRUE, colour = '#99000099',data=result_m_d %>% filter(NA_y=='2019'))
...
```{r}
result_chry <- result_m_d %>%
  group_by(NA_y) %>%
  mutate(sum_avr = cumsum(平均気温), sum_high = cumsum(最高気温)) %>% 
  mutate(avr_over0 = if_else(condition = 平均気温>0, 平均気温, 0)) %>% 
  mutate(high_over0 = if_else(condition = 最高気温>0, 最高気温, 0)) %>% 
  mutate(sum_avr_over0 = cumsum(avr_over0)) %>% 
  mutate(sum_high_over0 = cumsum(high_over0))

result_avr_over_400 <- result_chry %>% filter(sum_avr_over0 >= 400)
# result_high_over_600 <- result_chry %>% filter(sum_high_over0 >= 600)
result_avr_over_400 %>%
  mutate(m_d_str = as.character(m_d)) %>% 
  group_by(NA_y) %>%
  summarise(avr_over0_over_400 = min(m_d_str))

result_chry %>% filter(NA_y == 1984)
```
...
g <- g + geom_point(mapping = this_aes, alpha=0.4, na.rm = TRUE, colour = '#99000099',data=result_stm_this)
# g <- g + geom_boxplot(mapping = aes_string(x=x_axis, y=y_axis, group='m_jun'), alpha=0.4, notch = TRUE, na.rm = TRUE, colour = '#00009999',data=result_m_d %>% filter(y_num > 2008) %>% filter(NA_y!='2019') %>% group_by(m_jun))
g <- g + theme(axis.text.x = element_text(angle = 90, hjust = 1))
g <- g + labs(title = str_c(y_axis,thls_year,'〜',sep=' '))
plot(g)
...
```{r}
quartz(type='pdf', file=str_c(y_axis,'.pdf'))
plot(g)
dev.off()
