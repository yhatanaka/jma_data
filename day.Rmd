...
# この年以降のデータ
thls_year = 1999
result_stm <- result_m_d %>% filter(y_num > thls_year) %>% mutate(eff_tmp_5 = if_else(condition = 平均気温>5, 平均気温-5, 0)) %>% mutate(eff_tmp_0 = if_else(condition = 平均気温>0, 平均気温, 0)) %>% mutate(eff_tmp_3 = if_else(condition = 平均気温>3, 平均気温-3, 0)) %>% mutate(eff_h_tmp_5 = if_else(condition = 最高気温>5, 最高気温-5, 0))
result_stm %<>% group_by(NA_y) %>% mutate(stm_5=cumsum(eff_tmp_5)) %>% mutate(stm_0=cumsum(eff_tmp_0)) %>% mutate(stm_3=cumsum(eff_tmp_3)) %>% mutate(stm_h_5=cumsum(eff_h_tmp_5))
