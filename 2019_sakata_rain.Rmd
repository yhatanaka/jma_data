...
data_file = '1969-2019_02-06_rain_10d.csv'
...
result_m_d <- named_tbl_1 %>% mutate(m_d = str_c(start_m %>% padding_date,start_d %>% padding_date,sep='/') %>% as.factor)
...
g <- g + geom_point(mapping = this_aes, alpha=0.4, na.rm = TRUE, colour = '#99000099',data=result_m_d %>% filter(start_y=='2019'))

g <- g + geom_boxplot(mapping = aes_string(x=x_axis, y=y_axis, group='m_d'), alpha=0.4, notch = TRUE, na.rm = TRUE, colour = '#00009999',data=result_m_d %>% filter(start_y!='2019') %>% group_by(m_d))
...
```{r}
quartz(type='pdf', file='2019_rain.pdf')
plot(g)
dev.off()
...
```{r}
result_m_d %>% ungroup()
result_m_d %>% filter(start_m != '06') %>% filter(m_d != '05/21') %>% filter(m_d != '02/01') %>% filter(m_d != '02/11') %>% group_by(start_y) %>% summarise(sum_rain = sum(降水量)) %>% filter(sum_rain < 200)
# result_m_d %>% filter(m_d != '06/21') %>% filter(m_d != '06/11') %>% group_by(start_y) %>% summarise(sum_rain = sum(降水量)) %>% filter(sum_rain < 350)
# result_m_d %>% filter(as.numeric(as.character(start_m)) > 4) %>% group_by(start_y) %>% summarise(sum_rain = sum(降水量))
```
